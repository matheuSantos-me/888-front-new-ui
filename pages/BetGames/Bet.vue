...
<el-row class="width-full">
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    <div class="panel-heading games-navigation p-0 height-5" :class="{'bg-orange12-page': layoutType == 'layout2'}">
      ...
      <MiniHeader v-else :miniGameType="29" />
    </div>
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
      ...
      <div class="bg-black-darker">
        <div class="panel-collapse collapse in playerbox3 lotusbg position-relative" style="margin:0px; padding:0px;" id="bet-game-frame-container" ref="betgameframecontainer">
					<iframe style="width:100%; height:646px;" frameborder="0" scrolling="no" :src="'/html/index3.html'" name="datacont1" id="myframe" class="bet-game-frame"></iframe>
      ...
      <div class="col-md-12 m-b-15" :class="isMobile ? 'p-0' : 'p-t-0'">
        <div class="col-md-12" :class="[{'p-0': isMobile}, `v-box1-agero-prematch-${themeType}`]">
                    ...
                    <tr>
                      <td v-if="!isShowMobile" width="360">
                        <div class=" m-l-10 pull-left table-td-valign-middle">
                          <div class="f-s-25 p-l-20"><i class="fa fa-star m-r-8 text-orange11"></i><strong id="game_title">LIVE WAR</strong></div>
                          <div class="f-s-12 p-l-50 text-gray">플레이어 vs 딜러가 카드한장으로 하는 라이브게임</div>
                        </div>
                      </td>
                              ...
                              <tr>
                                <td class="width-half f-s-12 text-center p-5 p-t-10">
                                    ...
                                    <div class="progress progress-striped bg-black-darker2 height-15 m-t-10">
                                      <div class="progress-bar width-full height-15 progress-bar-inverse"></div>
                                    </div>
                                </td>
                                <td class="width-half f-s-12 text-center p-5 p-t-10">
                                    ...
                                    <div class="progress progress-striped bg-black-darker2 height-15 m-t-10 active">
                                      <div class="progress-bar width-full height-15">
                                          <div id="timercounter" class="p-0 m-0 m-t-m-2 text-black f-s-14"><strong>카드가 처리되는 중입니다. 잠시 기다려주세요.</strong></div>
                                      </div>
                                    </div>
                                </td>
                              </tr>
          ...
          <div v-if="currentGameInfo" class="col-md-12 position-relative" :class="[{'p-0': isMobile}, `v-box2-agero-${themeType}`]">
            <ul class="col-md-12 width-full m-t-12 m-b-1 p-0 p-l-2 display-none">
              <button type="button" class="btn btn-block btnstyle1 btnstyle1-inverse3 btnstyle1-bg text-white f-s-14 text-left p-10" data-toggle="collapse" data-target="#Collapse_1"><i class="fa fa-chevron-circle-down m-r-5 text-grey901"></i><strong><span>메인 (MAIN BET)</span></strong></button>
            </ul>
            <div class="panel-collapse col-md-12 m-0 p-0 collapse show" :class="{'m-t-20': !isMobile}" id="Collapse_1">
              <ul class="list-box-g-agero col-md-12 width-full m-t-0 m-b-12">
                  <li :class="isMobile ? 'hva_bt_sub' : 'hva_bt_sub1-agero'">
                    <button type="button" class="btn btn-btn btn-sports btn-block" :class="[isMobile ? 'f-s-13' : 'p-t-30 p-b-30 f-s-15', `btn-${themeType}`, {'disabled': !isBetAvailable || currentGameInfo.odds[0].status != 'active'}]" @click="onClickBetButton(0, currentGameInfo.odds[0].value)">
                      <ul class="list-box-g-agero col-md-12 m-3">
                        <li v-if="!isMobile" class="hva-txt text-center text-white f-w">
                          <div class="m-b-10 f-s-18"><i class="fa fa-star-o m-r-5 text-gray"></i><span>딜러(Dealer)승리</span></div>
                          <div class="f-s-20">
                            <strong>
                        ...
                        <li v-if="isMobile" class="hva-txt text-left text-gray"><i class="fa fa-star-o m-r-5 text-white"></i><span>딜러(Dealer)승리</span></li>
                        <li v-if="isMobile" class="hva-txt text-right text-white f-s-15 p-r-10">
                          <strong>
                            <span v-if="isBetAvailable && currentGameInfo.odds[0].status == 'active'">{{currentGameInfo.odds[0].value}}</span>
                            <span v-else><img :src="'/image/mini/ajax-loader.gif'"></span>
                          </strong>
                  ...
                  <li :class="isMobile ? 'hva_bt_sub' : 'hva_bt_sub1-agero'">
                    <button type="button" class="btn btn-btn btn-sports btn-block" :class="[isMobile ? 'f-s-13' : 'p-t-30 p-b-30 f-s-15', `btn-${themeType}`, {'disabled': !isBetAvailable || currentGameInfo.odds[2].status != 'active'}]" @click="onClickBetButton(1, currentGameInfo.odds[2].value)">
                      <ul class="list-box-g-agero col-md-12 m-3">
                        <li v-if="!isMobile" class="hva-txt text-center text-white f-w">
                            <div class="m-b-10 f-s-18"><i class="fa fa-star-o m-r-5 text-gray"></i><span>워(War)</span></div>
                            <div class="f-s-20">
                              <strong>
                        ...
                        <li v-if="isMobile" class="hva-txt text-left text-gray"><i class="fa fa-star-o m-r-5 text-white"></i><span>워(War)</span></li>
                        <li v-if="isMobile" class="hva-txt text-right text-white f-s-15 p-r-10">
                          <strong>
                            <span v-if="isBetAvailable && currentGameInfo.odds[2].status == 'active'">{{currentGameInfo.odds[2].value}}</span>
                            <span v-else><img :src="'/image/mini/ajax-loader.gif'"></span>
                  ...
                  <li :class="isMobile ? 'hva_bt_sub' : 'hva_bt_sub1-agero'">
                    <button type="button" class="btn btn-btn btn-sports btn-block" :class="[isMobile ? 'f-s-13' : 'p-t-30 p-b-30 f-s-15', `btn-${themeType}`, {'disabled': !isBetAvailable || currentGameInfo.odds[1].status != 'active'}]" @click="onClickBetButton(2, currentGameInfo.odds[1].value)">
                      <ul class="list-box-g-agero col-md-12 m-3">
                        <li v-if="!isMobile" class="hva-txt text-center text-white f-w">
                            <div class="m-b-10 f-s-18"><i class="fa fa-star-o m-r-5 text-gray"></i><span>플레이어(Player)승리</span></div>
                            <div class="f-s-20">
                              <strong>
                        ...
                        <li v-if="isMobile" class="hva-txt text-left text-gray"><i class="fa fa-star-o m-r-5 text-white"></i><span>플레이어(Player)승리</span></li>
                        <li v-if="isMobile" class="hva-txt text-right text-white f-s-15 p-r-10">
                          <strong>
                            <span v-if="isBetAvailable && currentGameInfo.odds[1].status == 'active'">{{currentGameInfo.odds[1].value}}</span>
                            <span v-else><img :src="'/image/mini/ajax-loader.gif'"></span>
            ...
            <div v-show="!isBetAvailable" class="mini-bet-lock display-center">
              <img :src="layoutType == 'layout2' ? '/image/mini/Ic_lock.png' : '/image/mini/Lock-Icon.png'" :width="layoutType == 'layout2' ? 48 : 100">
            </div>
          </div>
        </div>
		  </div>
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">벳게임 1:1벳 배팅내역</div>
          </el-col>
        </el-row>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      BetGameBetBet: "getBetGameBetBet",
  ...
  data() {
    return {
      ...
      gameName: "벳게임 1:1벳",                                     // 게임명
      gameType: "BetGameBet",                                      // 게임타입
      betLogs: null,                                               // 배팅내역
      isCheck: true,
      initFrameWidth: 0,
      isShowMobile: false
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.BetGameBet.gameStatus_auto || val.BetGameBet.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.BetGameBet.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'BetGames-Bet')
    ...
    let levelSetting = this.gameSettings.BetGameBet.levelSetting[this.gameSettings.BetGameBet.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.BetGameBet.gameStatus_auto | this.gameSettings.BetGameBet.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.BetGameBet.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameBetGameBetRound'),
    mainService.on('miniGameBetGameBetRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameBetGameBetOdds')
    mainService.on('miniGameBetGameBetOdds', (state) => {
    ...
    async getInitInfo() {
      try {
        ...
        if(!this.currentGameInfo) {
          this.currentGameInfo = { times: null, round: null }
