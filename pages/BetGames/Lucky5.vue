...
<el-row class="width-full">
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    <div class="panel-heading games-navigation p-0 height-5" :class="{'bg-orange12-page': layoutType == 'layout2'}">
      ...
      <MiniHeader v-else :miniGameType="34" />
    </div>
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
      ...
      <div class="bg-black-darker">
        <div class="panel-collapse collapse in playerbox3 lotusbg position-relative" style="margin:0px; padding:0px;" id="bet-game-frame-container" ref="betgameframecontainer">
					<iframe style="width:100%; height:646px;" frameborder="0" scrolling="no" :src="'/html/index10.html'" name="datacont1" id="myframe" class="bet-game-frame"></iframe>
      ...
      <div class="col-md-12 m-b-15" :class="isMobile ? 'p-0' : 'p-t-0'">
        <div class="col-md-12" :class="[{'p-0': isMobile}, `v-box1-agero-prematch-${themeType}`]">
                ...
                <tr>
                  <td>
                    <div class=" m-l-10 pull-left table-td-valign-middle">
                      <div v-if="isShowMobile" class="f-s-20"><strong id="game_title">LUCKY5</strong><br><strong class="text-orange11">{{currentGameInfo ? currentGameInfo.drawCode : 0}}</strong></div>
                      <div v-if="!isShowMobile" class="f-s-25 p-l-20"><i class="fa fa-star m-r-8 text-orange11"></i><strong id="game_title">LUCKY5</strong> - <strong class="text-orange11" id="lucky5_game_tnum_set">{{currentGameInfo ? currentGameInfo.drawCode : 0}}</strong><strong>회차</strong></div>
                      <div v-if="!isShowMobile" class="f-s-12 p-l-50 text-gray">4분마다 36개의 공 중에서 5개의 공을 추첨하는 게임</div>
                    </div>
                  </td>
          ...
          <div class="col-md-12 " :class="[{'p-0': isMobile}, `v-box2-agero-${themeType}`]">
            <ul class="col-md-12 width-full m-t-12 m-b-1 p-0 p-l-2">
              <button type="button" class="btn btn-block btnstyle1 btnstyle1-inverse3 btnstyle1-bg text-white f-s-14 text-left p-10" data-toggle="collapse" data-target="#Collapse_1">
                <i class="fa fa-chevron-circle-down m-r-5 text-grey901"></i><strong><span>당첨번호 (홀/짝)</span></strong>
              </button>
            </ul>
            ...
            <ul class="col-md-12 width-full m-b-1 p-0 p-l-2">
              <button type="button" class="btn btn-block btnstyle1 btnstyle1-inverse3 btnstyle1-bg text-white f-s-14 text-left p-10" data-toggle="collapse" data-target="#Collapse_2">
                <i class="fa fa-chevron-circle-down m-r-5 text-grey901"></i><strong><span>당첨번호 (오버/언더)</span></strong>
              </button>
            </ul>
              ...
              <ul class="list-box-g-agero col-md-12 width-full m-t-0 m-b-12">
                <li :class="isMobile ? 'hva_bt_sub' : 'hva_bt_sub2'">
                  <button type="button" class="btn btn-btn btn-sports btn-block f-s-13" :class="`btn-${themeType}`" @click="onClickBetButton(2, 1.9)">
                    <ul class="list-box-g-agero col-md-12 m-3">
                      <li class="hva-txt text-left text-gray"><i class="fa fa-star-o m-r-5 text-white"></i><span>당첨번호 총 합이 92.5 오버</span></li>
                      <li class="hva-txt text-right text-white f-s-15 p-r-10"><strong><span>1.90</span></strong></li>
                    </ul>
                  </button>
                </li>
                <li :class="isMobile ? 'hva_bt_sub' : 'hva_bt_sub2'">
                  <button type="button" class="btn btn-btn btn-sports btn-block f-s-13" :class="`btn-${themeType}`" @click="onClickBetButton(3, 1.9)">
                    <ul class="list-box-g-agero col-md-12 m-3">
                      <li class="hva-txt text-left text-gray"><i class="fa fa-star-o m-r-5 text-white"></i><span>당첨번호 총 합이 92.5 언더</span></li>
                      <li class="hva-txt text-right text-white f-s-15 p-r-10"><strong><span>1.90</span></strong></li>
                    </ul>
                  </button>
                </li>
      ...
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">벳게임 럭키5 배팅내역</div>
          </el-col>
        </el-row>
...
const popupRouter = "BetGameLucky5"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      BetGameLucky5Bet: "getBetGameLucky5Bet",
  ...
  data() {
    return {
      ...
      gameName: "벳게임 럭키5",                                    // 게임명
      gameType: "BetGameLucky5",                                  // 게임타입
      betLogs: null,                                              // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    BetGameLucky5Bet(val) {
      this.isBetAvailable = !Boolean(val)
    },
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.BetGameLucky5.gameStatus_auto || val.BetGameLucky5.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.BetGameLucky5.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'BetGames-Lucky5')
    ...
    let levelSetting = this.gameSettings.BetGameLucky5.levelSetting[this.gameSettings.BetGameLucky5.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.BetGameLucky5.gameStatus_auto | this.gameSettings.BetGameLucky5.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.BetGameLucky5.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameBetGameLucky5Round'),
    mainService.on('miniGameBetGameLucky5Round', ( currentGameInfo ) => {
      if(currentGameInfo) {
