    ...
    <div class="panel-body height-100-pro" :class="[{'p-l-14 p-r-14': !isMobile}, getPageBackColor(themeType)]" style="position:relative;">
      <el-row v-if="currentUser">
        ...
        <el-row class="col-md-12 p-0 m-t-14">
          <div :class="[gameType === 'LotusBaccarat1' ? 'on' : '', `gametype-button-${themeType}`]" @click="onClickGameType('LotusBaccarat1')">바카라1</div>
          <div :class="[gameType === 'LotusEvenOdd' ? 'on' : '', `gametype-button-${themeType}`]" @click="onClickGameType('LotusEvenOdd')">홀짝</div>
        </el-row>
        <!-- 배팅내역 서브 탭 끝 -->
        <!-- 배팅내역 -->
...
export default {
  ...
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'BettingHistory-Lotus')
    // 파워볼 배팅내역 정산후 결과 패치
