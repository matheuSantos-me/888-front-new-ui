  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
        ...
        <div class="col-md-12" :class="[{'p-0': isMobile}, `v-box1-agero-prematch-${themeType}`]">
          <div class="list-box-g-agero col-md-12 width-full m-b-10  clockbox bg-black16">
            <div class="pull-left table-td-valign-middle" :class="isMobile ? 'm-t-20 m-l-0' : 'm-t-13 m-l-10'">
              <div v-if="!isMobile" class="f-s-25 p-l-20">
                <i class="fa fa-star m-r-8 text-orange11"></i><strong>MGM(홀짝)</strong> - <strong class="text-orange11">{{currentGameInfo ? currentGameInfo.roundNumber : 0}}</strong><strong></strong><strong>회차</strong>
              </div>
          ...
          <div class="col-md-12" :class="[{'p-5': isMobile}, `v-box2-agero-${themeType}`]">
            <div class="col-md-12 p-0">
              <div class="col-md-12 p-0" :class="{'p-r-10': !isMobile}">
                ...
                <ul class="list-box-g-agero col-md-12 text-ellipsis m-b-5">
                  <li class="etc-live p-r-1">
                    <button type="button" class="btn btn-btn btn-block btnstyle1 btnstyle1-inverse3 btnstyle1-bg p-t-10 p-b-10 btn-sports" :class="`btn-${themeType}`" @click="onClickBetButton('odd', gameSettings.MGMEvenOdd.oeOdds)">
                      <ul class="list-box-mg">
                        <div class="hva-txt text-blue902"><center><div class="rounded-corner bg-black21 width-70 height-70 f-s-22 p-t-10"><strong><span class="display-none">MGM 홀짝</span><span>홀</span><div class="f-s-15 m-t-5 text-white"><span>{{gameSettings.MGMEvenOdd.oeOdds | filterOdds()}}</span></div></strong></div></center></div>
                      </ul>
                    </button>
                  </li>
                  <li class="etc-live p-l-1">
                    <button type="button" class="btn btn-btn btn-block btnstyle1 btnstyle1-inverse3 btnstyle1-bg p-t-10 p-b-10 btn-sports" :class="`btn-${themeType}`" @click="onClickBetButton('even', gameSettings.MGMEvenOdd.oeOdds)"> 
                      <ul class="list-box-mg">
                        <div class="hva-txt text-red905"><center><div class="rounded-corner bg-black21 width-70 height-70 f-s-22 p-t-10"><strong><span class="display-none">MGM 홀짝</span><span>짝</span><div class="f-s-15 m-t-5 text-white"><span>{{gameSettings.MGMEvenOdd.oeOdds | filterOdds()}}</span></div></strong></div></center></div>
                      </ul>
                    </button>
                  </li>
                </ul>
            ...
            <div v-show="!isBetAvailable" class="mini-bet-lock display-center">
              <img :src="layoutType == 'layout2' ? '/image/mini/Ic_lock.png' : '/image/mini/Lock-Icon.png'" width="100px">
            </div>
          </div>
        </div>
      </div>
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">MGM 홀짝 배팅내역</div>
          </el-col>
        </el-row>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      MGMEvenOddBet: "getMGMEvenOddBet"
    })
  },
  data() {
    return {
      isBetAvailable: true,                                       // 배팅가능상태
      baseUrl: "https://mgm.nutv265.com/oddeven.html",  // MGM 홀짝 중계 URL
      remainTime: "00:00",                                        // 현재 회차 남은시간
      currentGameInfo: null,                                      // 현재 회차정보
      gameName: "MGM 홀짝",                                  // 게임명
      gameType: "MGMEvenOdd",                                // 게임타입
      betLogs: null,                                              // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.MGMEvenOdd.gameStatus_auto || val.MGMEvenOdd.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.MGMEvenOdd.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'MGM-MGMEvenOdd')
    this.initframe()
    ...
    let levelSetting = this.gameSettings.MGMEvenOdd.levelSetting[this.gameSettings.MGMEvenOdd.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.MGMEvenOdd.gameStatus_auto | this.gameSettings.MGMEvenOdd.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.MGMEvenOdd.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameMGMEvenOddRound'),
    mainService.on('miniGameMGMEvenOddRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameMGMEvenOddTime')
    mainService.on('miniGameMGMEvenOddTime', (state) => {
      this.remainMins = 0
      this.remainSecs = 59
    })
  ...
  methods: {
    ...
    async initframe() {
      if (this.isMobile) {
        this.baseUrl = "https://mgm.nutv265.com/m/oddeven.html"
      } else {
        this.baseUrl = "https://mgm.nutv265.com/oddeven.html"
      }
