  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
              ...
              <el-row>
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-168" :class="[`mini-bet-button-keno-${themeType}`]" @click="onClickBetButton('left', gameSettings.BubbleLadder.lrOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/left.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.BubbleLadder.lrOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-168" :class="[`mini-bet-button-keno-${themeType}`]" @click="onClickBetButton('right', gameSettings.BubbleLadder.lrOdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-168" :class="[`mini-bet-button-keno-${themeType}`]" @click="onClickBetButton('three', gameSettings.BubbleLadder.lineOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/line3.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.BubbleLadder.lineOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-168" :class="[`mini-bet-button-keno-${themeType}`]" @click="onClickBetButton('four', gameSettings.BubbleLadder.lineOdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-168" :class="[`mini-bet-button-keno-${themeType}`]" @click="onClickBetButton('odd', gameSettings.BubbleLadder.oeOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/odd.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.BubbleLadder.oeOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-168" :class="[`mini-bet-button-keno-${themeType}`]" @click="onClickBetButton('even', gameSettings.BubbleLadder.oeOdds)">
                        <el-row class="m-t-20">
      ...
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">보글 사다리 배팅내역</div>
          </el-col>
        </el-row>
...
const popupRouter = "BubbleLadder"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      BubbleLadderBet: "getBubbleLadderBet",
  ...
  data() {
    return {
      ...
      baseUrl: "https://bepick.net/live/bubbleladder",                // 보글 사다리 중계 URL
      remainTime: "00:00",                                      // 현재 회차 남은시간
      currentGameInfo: null,                                    // 현재 회차정보
      gameName: "보글 사다리",                                   // 게임명
      gameType: "BubbleLadder",                                 // 게임타입
      betLogs: null,                                            // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.BubbleLadder.gameStatus_auto || val.BubbleLadder.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.BubbleLadder.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.BubbleLadder.isVisible || !this.gameSettings.Mini.bubble) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-BubbleLadder')
    this.initframe()
    ...
    let levelSetting = this.gameSettings.BubbleLadder.levelSetting[this.gameSettings.BubbleLadder.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.BubbleLadder.gameStatus_auto | this.gameSettings.BubbleLadder.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.BubbleLadder.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameBubbleLadderRound'),
    mainService.on('miniGameBubbleLadderRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameBubbleLadderTime')
    mainService.on('miniGameBubbleLadderTime', (state) => {
      this.remainMins = 2
      this.remainSecs = 59
    })
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogBubbleLadder')
    mainService.on('miniBetLogBubbleLadder', (betLog) => {
