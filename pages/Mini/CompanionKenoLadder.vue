  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
              ...
              <el-row>
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-row>
                      <div class="mini-bet-button-keno height-110" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('left', gameSettings.CompanionKenoLadder.LROdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/left.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.CompanionKenoLadder.LROdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-row>
                    <el-row class="m-t-5">
                      <div class="mini-bet-button-keno height-110" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('right', gameSettings.CompanionKenoLadder.LROdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-row>
                      <div class="mini-bet-button-keno height-110" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('three', gameSettings.CompanionKenoLadder.LineOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/line3.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.CompanionKenoLadder.LineOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-row>
                    <el-row class="m-t-5">
                      <div class="mini-bet-button-keno height-110" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('four', gameSettings.CompanionKenoLadder.LineOdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  <el-col :span="24">
                    <div class="mini-bet-button-keno height-54 f-s-13  p-t-7">
                      동행키노사다리의 경우 결과값 홀짝의 밸런스가 맞지 않아<br>
                      홀짝의 배팅은 지원하지 않습니다.
                    </div>
                  </el-col>
                </el-row>
      ...
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">동행키노사다리 배팅내역</div>
          </el-col>
        </el-row>
...
const popupRouter = "CompanionKenoLadder"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      CompanionKenoLadderBet: "getCompanionKenoLadderBet",
  ...
  data() {
    return {
      ...
      baseUrl: "https://bepick.net/live/speedkenoladder/scrap",   // 동행키노사다리 중계 URL
      remainTime: "00:00",                                      // 현재 회차 남은시간
      currentGameInfo: null,                                    // 현재 회차정보
      gameName: "동행키노사다리",                                    // 게임명
      gameType: "CompanionKenoLadder",                                  // 게임타입
      betLogs: null,                                            // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    CompanionKenoLadderBet(val) {
      this.isBetAvailable = !Boolean(val)
    },
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.CompanionKenoLadder.gameStatus_auto || val.CompanionKenoLadder.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.CompanionKenoLadder.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.CompanionKenoLadder.isVisible || !this.gameSettings.Mini.companion) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-CompanionKenoLadder')
    this.initframe()
    ...
    let levelSetting = this.gameSettings.CompanionKenoLadder.levelSetting[this.gameSettings.CompanionKenoLadder.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.CompanionKenoLadder.gameStatus_auto | this.gameSettings.CompanionKenoLadder.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.CompanionKenoLadder.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameCompanionKenoLadderRound'),
    mainService.on('miniGameCompanionKenoLadderRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameCompanionKenoLadderTime')
    mainService.on('miniGameCompanionKenoLadderTime', (state) => {
      this.remainMins = 4
      this.remainSecs = 59
    })
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogCompanionKenoLadder')
    mainService.on('miniBetLogCompanionKenoLadder', (betLog) => {
