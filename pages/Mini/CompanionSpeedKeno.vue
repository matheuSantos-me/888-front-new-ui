...
<el-row class="width-full">
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    <div class="panel-heading games-navigation p-0 height-5" :class="{'bg-orange12-page': layoutType == 'layout2'}">
      <MiniHeader :miniGameType="48" />
    </div>
    <div class="panel-body position-relative height-full" 
      :class="`mini-page-bg-${themeType}`" data-height="100%">
          ...
          <el-col class="mini-bubble-bet-panel">
            <el-row>
              <el-row class="p-5 height-50">
                ...
                <el-col class="height-full bg-brown1 f-w-700 display-center" :class="isMobile ? 'f-s-12' : 'f-s-15'" style="border-right: 2px solid #996148" :span="8">
                  {{currentGameInfo ? currentGameInfo.round : 0}}회차 ({{currentGameInfo ? currentGameInfo.times : 0}})
                </el-col>
                <el-col class="height-full bg-brown text-brown f-w-700 display-center" style="border-left: 2px solid #996148" :span="4">
                  {{miniBettingEndText}}
                </el-col>
              ...
              <el-row>
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile? 24 : 5">
                    <div class="mini-bet-type-name" :style="isMobile  ? '' : 'height: 260px;'">
                      시작
                    </div>
                  </el-col>
                  <el-col :span="isMobile? 24 : 19">
                    <el-row>
                      <div class="mini-bet-button-keno height-127" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('odd', gameSettings.CompanionSpeedKeno.OEOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/odd.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-20 f-w-700">홀</span>
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.CompanionSpeedKeno.OEOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-row>
                    <el-row class="m-t-5">
                      <div class="mini-bet-button-keno height-127" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('even', gameSettings.CompanionSpeedKeno.OEOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/even.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-20 f-w-700">짝</span>
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.CompanionSpeedKeno.OEOdds | filterOdds()}}</span>
                        </el-row>
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-row>
                      <div class="mini-bet-button-keno height-127" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('under', gameSettings.CompanionSpeedKeno.UOOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/under.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-20 f-w-700">언더</span>
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.CompanionSpeedKeno.UOOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-row>
                    <el-row class="m-t-5">
                      <div class="mini-bet-button-keno height-127" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('over', gameSettings.CompanionSpeedKeno.UOOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/over.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-20 f-w-700">오버</span>
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.CompanionSpeedKeno.UOOdds | filterOdds()}}</span>
                        </el-row>
      ...
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">동행스피드키노 배팅내역</div>
          </el-col>
        </el-row>
...
const popupRouter = "CompanionSpeedKeno"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      CompanionSpeedKenoBet: "getCompanionSpeedKenoBet",
  ...
  data() {
    return {
      ...
      baseUrl: "https://bepick.net/live/speedkeno/scrap",   // 동행스피드키노 중계 URL
      remainTime: "00:00",                                    // 현재 회차 남은시간
      currentGameInfo: null,                                  // 현재 회차정보
      gameName: "동행스피드키노",                                  // 게임명
      gameType: "CompanionSpeedKeno",                              // 게임타입
      betLogs: null,                                          // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    CompanionSpeedKenoBet(val) {
      this.isBetAvailable = !Boolean(val)
    },
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.CompanionSpeedKeno.gameStatus_auto || val.CompanionSpeedKeno.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.CompanionSpeedKeno.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.CompanionSpeedKeno.isVisible || !this.gameSettings.Mini.companion) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-CompanionSpeedKeno')
    this.initframe()
    ...
    let levelSetting = this.gameSettings.CompanionSpeedKeno.levelSetting[this.gameSettings.CompanionSpeedKeno.levelSetting.findIndex(setting => setting.level == userLevel)]
    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.CompanionSpeedKeno.gameStatus_auto | this.gameSettings.CompanionSpeedKeno.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.CompanionSpeedKeno.gameStatus_manual)
    ...
    mainService.off('miniGameCompanionSpeedKenoRound'),
    mainService.on('miniGameCompanionSpeedKenoRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameCompanionSpeedKenoTime')
    mainService.on('miniGameCompanionSpeedKenoTime', (state) => {
      this.remainMins = 4
      this.remainSecs = 59
    })
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogCompanionSpeedKeno')
    mainService.on('miniBetLogCompanionSpeedKeno', (betLog) => {
  ...
  methods: {
    ...
    onClickBetButton(betType, odds) {
      ...
      let data = {
        ...
        betTypeName: this.gameName + "-" + this.currentGameInfo.round + " (" + this.currentGameInfo.times + ") 회차",
