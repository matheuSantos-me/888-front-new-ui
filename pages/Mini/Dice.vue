...
export default {
  ...
  watch: {
    gameSettings(val) {
      this.isCheck = !Boolean(val.gameStatus_socket || val.CoreDice.gameStatus_manual)
    },
    screenWidth(val) {
      this.initframe()
    }
  ...
  created() {
    if (!this.gameSettings.CoreDice.isVisible || !this.gameSettings.Mini.sportcore) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-Dice')
    this.initframe()
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0)
    this.$store.commit("SET_MINIGAMETYPE", 7)

    // 게임셋팅정보 배팅슬립정보 설정
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.CoreDice.gameStatus_manual)
  ...
  methods: {
    ...
    async getInitInfo() {
      try {
        let res = await minigameInfoService.find({ query: { type: 2 }})
