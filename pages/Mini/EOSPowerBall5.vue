  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
        ...
        <div class="panel-collapse collapse in playerbox3 minibg">
          ...
          <el-col class="mini-bet-panel">
            <el-row>
              ...
              <el-row v-if="gameSettings.EOSPowerBall5.canBetMix" style="border-top: 5px solid #f4e1c0;">
                <el-col :span="12">
              ...
              <el-row v-if="betType === 0">
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name" :class="{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}">
                      {{gameSettings.EOSPowerBall5.canBetMix ? '파워볼' : '파워볼 홀짝'}}
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('power-odd', gameSettings.EOSPowerBall5.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/odd.png'"></el-row>
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('power-even', gameSettings.EOSPowerBall5.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/even.png'"></el-row>
                    </div>
                  </el-col>
                  <el-col v-if="gameSettings.EOSPowerBall5.canBetNumber" :span="getSpan()">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-under', gameSettings.EOSPowerBall5.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/under.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">4.5</el-row>
                    </div>
                  </el-col>
                  <el-col v-if="gameSettings.EOSPowerBall5.canBetNumber" :span="getSpan()">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-over', gameSettings.EOSPowerBall5.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/over.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">4.5</el-row>
                    </div>
                  </el-col>
                </el-row>
                <el-row v-if="!gameSettings.EOSPowerBall5.canBetNumber" class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name" :class="{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}">
                      {{gameSettings.EOSPowerBall5.canBetNumber ? '파워볼' : '파워볼 언더오버'}}
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('power-under', gameSettings.EOSPowerBall5.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/under.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">4.5</el-row>
                    </div>
                  ...
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('power-over', gameSettings.EOSPowerBall5.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/over.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">4.5</el-row>
                    </div>
                ...
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name" :class="{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}">
                      {{gameSettings.EOSPowerBall5.canBetNumber ? '일반볼' : '일반볼 홀짝'}}
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-odd', gameSettings.EOSPowerBall5.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/odd.png'"></el-row>
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-even', gameSettings.EOSPowerBall5.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/even.png'"></el-row>
                    </div>
                  </el-col>
                  <el-col v-if="gameSettings.EOSPowerBall5.canBetNumber" :span="getSpan()">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-under', gameSettings.EOSPowerBall5.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/under.png'"></el-row>
                  ...
                  <el-col v-if="gameSettings.EOSPowerBall5.canBetNumber" :span="getSpan()">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-over', gameSettings.EOSPowerBall5.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/over.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">72.5</el-row>
                    </div>
                ...
                <el-row v-if="!gameSettings.EOSPowerBall5.canBetNumber" class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name" :class="{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}">
                      {{gameSettings.EOSPowerBall5.canBetNumber ? '일반볼' : '일반볼 언더오버'}}
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-under', gameSettings.EOSPowerBall5.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/under.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">72.5</el-row>
                    </div>
                  ...
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-over', gameSettings.EOSPowerBall5.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.EOSPowerBall5.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/over.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">72.5</el-row>
                    </div>
                ...
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name word-keep" :class="{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}">
                      일반볼 구간별
                    </div>
                  </el-col>
                  <el-col :class="isMobile ? 'width-33pro' : 'width-27pro'">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-big', gameSettings.EOSPowerBall5.normalBallSize.high)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallSize.high | filterOdds()}}</el-row>
                      <el-row class="f-s-23 f-w-700 m-t-5">대</el-row>
                      <el-row class="mini-bet-base">81-130</el-row>
                      <img class="float-left mini-bet-img-size m-l-1" :class="{'m-t-m-14': !gameSettings.EOSPowerBall5.canBetNumber}" :src="'/image/mini/big.png'">
                    </div>
                  </el-col>
                  <el-col :class="isMobile ? 'width-33pro' : 'width-27pro'">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-middle', gameSettings.EOSPowerBall5.normalBallSize.normal)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallSize.normal | filterOdds()}}</el-row>
                      <el-row class="f-s-23 f-w-700 m-t-5">중</el-row>
                      <el-row class="mini-bet-base">65-80</el-row>
                      <img class="float-left mini-bet-img-size m-l-1" :class="{'m-t-m-14': !gameSettings.EOSPowerBall5.canBetNumber}" :src="'/image/mini/middle.png'">
                    </div>
                  </el-col>
                  <el-col :class="isMobile ? 'width-33pro' : 'width-27pro'">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.EOSPowerBall5.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-small', gameSettings.EOSPowerBall5.normalBallSize.low)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallSize.low | filterOdds()}}</el-row>
                      <el-row class="f-s-23 f-w-700 m-t-5">소</el-row>
                      <el-row class="mini-bet-base">15-64</el-row>
                      <img class="float-left mini-bet-img-size m-l-1" :class="{'m-t-m-14': !gameSettings.EOSPowerBall5.canBetNumber}" :src="'/image/mini/small.png'">
                    </div>
                  </el-col>
                </el-row>
                <el-row v-if="gameSettings.EOSPowerBall5.canBetNumber" class="mini-bet-row">
                  ...
                  <el-col :span="isMobile ? 24 : 20">
                    <el-row>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-0', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/0.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-1', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/1.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-2', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/2.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-3', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/3.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-4', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                    ...
                    <el-row class="m-t-10">
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-5', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/5.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-6', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/6.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-7', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/7.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-8', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/8.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-9', gameSettings.EOSPowerBall5.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBallNumber | filterOdds()}}</el-row>
              ...
              <el-row v-else>
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-odd-over', gameSettings.EOSPowerBall5.powerBall_OO_EU)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBall_OO_EU | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-over.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+오버</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-odd-under', gameSettings.EOSPowerBall5.powerBall_OU_EO)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBall_OU_EO | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-under.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+언더</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-even-over', gameSettings.EOSPowerBall5.powerBall_OU_EO)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBall_OU_EO | filterOdds()}}</el-row>
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-even-under', gameSettings.EOSPowerBall5.powerBall_OO_EU)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.powerBall_OO_EU | filterOdds()}}</el-row>
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-over', gameSettings.EOSPowerBall5.normalBall_OO_EU)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBall_OO_EU | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-over.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+오버</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-under', gameSettings.EOSPowerBall5.normalBall_OU_EO)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBall_OU_EO | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-under.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+언더</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-over', gameSettings.EOSPowerBall5.normalBall_OU_EO)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBall_OU_EO | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/even-over.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">짝+오버</el-row>
                    </div>
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-under', gameSettings.EOSPowerBall5.normalBall_OO_EU)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBall_OO_EU | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/even-under.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">짝+언더</el-row>
                    </div>
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('odd-odd', gameSettings.EOSPowerBall5.mixBall)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.mixBall | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-odd.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+홀</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('odd-even', gameSettings.EOSPowerBall5.mixBall)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.mixBall | filterOdds()}}</el-row>
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('even-odd', gameSettings.EOSPowerBall5.mixBall)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.mixBall | filterOdds()}}</el-row>
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('even-even', gameSettings.EOSPowerBall5.mixBall)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.mixBall | filterOdds()}}</el-row>
                  ...
                  <el-col :span="isMobile ? 24 : 20">
                    <el-row>
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-small', gameSettings.EOSPowerBall5.normalBallMix.small)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallMix.small | filterOdds()}}</el-row>
                          <el-row class="m-t-5"><img :src="'/image/mini/odd-small.png'"></el-row>
                          <el-row class="f-s-14 f-w-700">홀+소</el-row>
                        </div>
                      </el-col>
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-middle', gameSettings.EOSPowerBall5.normalBallMix.middle)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallMix.middle | filterOdds()}}</el-row>
                          <el-row class="m-t-5"><img :src="'/image/mini/odd-middle.png'"></el-row>
                          <el-row class="f-s-14 f-w-700">홀+중</el-row>
                        </div>
                      </el-col>
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-big', gameSettings.EOSPowerBall5.normalBallMix.big)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallMix.big | filterOdds()}}</el-row>
                    ...
                    <el-row class="m-t-10">
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-small', gameSettings.EOSPowerBall5.normalBallMix.small)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallMix.small | filterOdds()}}</el-row>
                      ...
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-middle', gameSettings.EOSPowerBall5.normalBallMix.middle)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallMix.middle | filterOdds()}}</el-row>
                      ...
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-big', gameSettings.EOSPowerBall5.normalBallMix.big)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.EOSPowerBall5.normalBallMix.big | filterOdds()}}</el-row>
      ...
      <el-row v-show="currentUser" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`" @click="onClickHistoryTab(0)">EOS 파워볼 5분 배팅내역</div>
          </el-col>
        </el-row>
...
const popupRouter = "EOSPowerBall5"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      EOSPowerBall5Bet: "getEOSPowerBall5Bet",
  ...
  watch: {
    EOSPowerBall5Bet(val) {
      this.isBetAvailable = !Boolean(val)
    },
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.EOSPowerBall5.gameStatus_auto || val.EOSPowerBall5.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.EOSPowerBall5.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.EOSPowerBall5.isVisible || !this.gameSettings.Mini.ntry) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-EOSPowerBall5')
    this.initframe()
    ...
    let levelSetting = this.gameSettings.EOSPowerBall5.levelSetting[this.gameSettings.EOSPowerBall5.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.EOSPowerBall5.gameStatus_auto | this.gameSettings.EOSPowerBall5.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.EOSPowerBall5.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameEOSPowerBall5Round'),
    mainService.on('miniGameEOSPowerBall5Round', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameEOSPowerBall5Time')
    mainService.on('miniGameEOSPowerBall5Time', (state) => {
      this.remainMins = 4
      this.remainSecs = 59
    })
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogEOSPowerBall5')
    mainService.on('miniBetLogEOSPowerBall5', (betLog) => {
  ...
  methods: {
    ...
    // span 얻기
    getSpan() {
      if (this.gameSettings.EOSPowerBall5.canBetNumber) {
