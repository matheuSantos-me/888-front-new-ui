  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
              ...
              <el-row>
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile? 24 : 5">
                    <div class="mini-bet-type-name" :class="{'height-239': !isMobile}">
                      시작
                    </div>
                  </el-col>
                  <el-col :span="isMobile? 24 : 19">
                    <el-row>
                      <div class="mini-bet-button-keno height-117" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('left', gameSettings.NtryKLadder.LROdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/left.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.NtryKLadder.LROdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-row>
                    <el-row class="m-t-5">
                      <div class="mini-bet-button-keno height-117" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('right', gameSettings.NtryKLadder.LROdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-row>
                      <div class="mini-bet-button-keno height-117" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('three', gameSettings.NtryKLadder.LineOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/line3.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.NtryKLadder.LineOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-row>
                    <el-row class="m-t-5">
                      <div class="mini-bet-button-keno height-117" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('four', gameSettings.NtryKLadder.LineOdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  <el-col :span="24">
                    <div class="mini-bet-button-keno height-54 f-s-13  p-t-7">
                      키노사다리의 경우 결과값 홀짝의 밸런스가 맞지 않아<br>
                      홀짝의 배팅은 지원하지 않습니다.
                    </div>
                  </el-col>
                </el-row>
      ...
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">키노사다리 배팅내역</div>
          </el-col>
        </el-row>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      ntryKLadderBet: "getNtryKLadderBet",
  ...
  data() {
    return {
      ...
      baseUrl: "https://ntry.com/scores/keno_ladder/live.php",   // 키노사다리 중계 URL
      remainTime: "00:00",                                      // 현재 회차 남은시간
      currentGameInfo: null,                                    // 현재 회차정보
      gameName: "키노사다리",                                    // 게임명
      gameType: "NtryKLadder",                                  // 게임타입
      betLogs: null,                                            // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.NtryKLadder.gameStatus_auto || val.NtryKLadder.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.NtryKLadder.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.NtryKLadder.isVisible || !this.gameSettings.Mini.ntry) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-KenoLadder')
    this.initframe()
    ...
    let levelSetting = this.gameSettings.NtryKLadder.levelSetting[this.gameSettings.NtryKLadder.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.NtryKLadder.gameStatus_auto | this.gameSettings.NtryKLadder.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.NtryKLadder.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameNtryKLadderRound'),
    mainService.on('miniGameNtryKLadderRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameNtryKLadderTime')
    mainService.on('miniGameNtryKLadderTime', (state) => {
      this.remainMins = 4
      this.remainSecs = 59
    })
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogNtryKLadder')
    mainService.on('miniBetLogNtryKLadder', (betLog) => {
