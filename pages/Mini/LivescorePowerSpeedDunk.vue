  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
      ...
      <div class="bg-black-darker">
        <div class="panel-collapse collapse in livescore-playerbox3 minibg " id="CSEP_viewpan">
          <div id="mini-game-frame-container">
            <div id="livescore-game-frame-container-in">
              <iframe id="myframe" :src="baseUrl" width="854" height="746" scrolling="no" frameborder="0" class="mini-game-frame"></iframe>
            </div>
          </div>
          <div class="mini-ifram-lock"></div>
          <el-col class="mini-livescore-bet-panel">
            <el-row>
              ...
              <el-row>
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile? 24 : 5">
                    <div class="mini-bet-type-name" :class="{'height-213': !isMobile}">
                      넘버
                    </div>
                  </el-col>
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-213" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('first', gameSettings.LivescorePowerSpeedDunk.numberOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/first.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.LivescorePowerSpeedDunk.numberOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-213" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('second', gameSettings.LivescorePowerSpeedDunk.numberOdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-213" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('left', gameSettings.LivescorePowerSpeedDunk.lrOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/left.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.LivescorePowerSpeedDunk.lrOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-213" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('right', gameSettings.LivescorePowerSpeedDunk.lrOdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-213" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('success', gameSettings.LivescorePowerSpeedDunk.goalOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/success.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.LivescorePowerSpeedDunk.goalOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-213" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('unsuccess', gameSettings.LivescorePowerSpeedDunk.goalOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/unsuccess.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.LivescorePowerSpeedDunk.goalOdds | filterOdds()}}</span>
                        </el-row>
      ...
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">파워스피드덩크 배팅내역</div>
          </el-col>
        </el-row>
...
const popupRouter = "LivescorePowerSpeedDunk"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      LivescorePowerSpeedDunkBet: "getLivescorePowerSpeedDunkBet",
  ...
  data() {
    return {
      ...
      baseUrl: "https://livescore.co.kr/sports/score_board/new_game/game.php?t=powerspeed_dunk&link",   // 파워스피드덩크 중계 URL
      remainTime: "00:00",                                      // 현재 회차 남은시간
      currentGameInfo: null,                                    // 현재 회차정보
      gameName: "파워스피드덩크",                                    // 게임명
      gameType: "LivescorePowerSpeedDunk",                                  // 게임타입
      betLogs: null,                                            // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    LivescorePowerSpeedDunkBet(val) {
      this.isBetAvailable = !Boolean(val)
    },
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.LivescorePowerSpeedDunk.gameStatus_auto || val.LivescorePowerSpeedDunk.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.LivescorePowerSpeedDunk.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.LivescorePowerSpeedDunk.isVisible || !this.gameSettings.Mini.livescore) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-LivescorePowerSpeedDunk')
    this.initframe()
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0)
    this.$store.commit("SET_MINIGAMETYPE", 2)

    // 게임셋팅정보 배팅슬립정보 설정
    let userLevel = this.currentUser ? this.currentUser.level : 2
    let levelSetting = this.gameSettings.LivescorePowerSpeedDunk.levelSetting[this.gameSettings.LivescorePowerSpeedDunk.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.LivescorePowerSpeedDunk.gameStatus_auto | this.gameSettings.LivescorePowerSpeedDunk.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.LivescorePowerSpeedDunk.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameLivescorePowerSpeedDunkRound'),
    mainService.on('miniGameLivescorePowerSpeedDunkRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameLivescorePowerSpeedDunkTime')
    mainService.on('miniGameLivescorePowerSpeedDunkTime', (state) => {
      if (state == 'P') {
        this.remainMins = 2
      } else {
        this.remainMins = 1
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogLivescorePowerSpeedDunk')
    mainService.on('miniBetLogLivescorePowerSpeedDunk', (betLog) => {
  ...
  methods: {
    ...
    getBetNameFromType ( strTeamName ){
      ...
      switch (strTeamName){
        ...
        case 'first': strLabel += '1번 (넘버 맞추기)'; break;
        case 'second': strLabel += '2번 (넘버 맞추기)'; break;
        case 'success': strLabel += '성공 (성공/실패 맞추기)'; break;
        case 'unsuccess': strLabel += '실패 (성공/실패 맞추기)'; break;
      }
      return strLabel;
    },
    // 경기시간계산
