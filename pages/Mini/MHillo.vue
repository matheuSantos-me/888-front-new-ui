...
export default {
  ...
  watch: {
    gameSettings(val) {
      this.isCheck = !Boolean(val.gameStatus_socket || val.CoreMHillo.gameStatus_manual)
    },
    screenWidth(val) {
      this.initframe()
    }
  ...
  created() {
    if (!this.gameSettings.CoreMHillo.isVisible || !this.gameSettings.Mini.sportcore) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-MHillo')
    this.initframe()
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0)
    this.$store.commit("SET_MINIGAMETYPE", 7)

    // 게임셋팅정보 배팅슬립정보 설정
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.CoreMHillo.gameStatus_manual)
  ...
  methods: {
    ...
    async getInitInfo() {
      try {
        let res = await minigameInfoService.find({ query: { type: 3 }})
