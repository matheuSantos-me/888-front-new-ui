  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
          ...
          <el-col class="mini-bet-panel">
            <el-row>
              <el-row class="p-5 height-50">
                ...
                <el-col class="height-full bg-brown1 f-w-700 display-center" :class="isMobile ? 'f-s-12' : 'f-s-15'" style="border-right: 2px solid #996148" :span="8">{{currentGameInfo ? currentGameInfo.roundNumber : '00:00'}}</el-col>
              ...
              <el-row>
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-179" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('buy', gameSettings.NamedFX2.marketPriceOdds)">
                        <el-row class="m-t-20">
                          <img :src="'/image/mini/buy.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.NamedFX2.marketPriceOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-179" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('sell', gameSettings.NamedFX2.marketPriceOdds)">
                        <el-row class="m-t-20">
                          <img :src="'/image/mini/sell.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.NamedFX2.marketPriceOdds | filterOdds()}}</span>
                        </el-row>
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-179" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('odd', gameSettings.NamedFX2.oeOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/odd.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.NamedFX2.oeOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-179" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('even', gameSettings.NamedFX2.oeOdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-179" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('under', gameSettings.NamedFX2.uoOdds)">
                        <el-row class="m-t-20">
                          <img class="mini-bet-lad-img" :src="'/image/mini/under.png'">
                        </el-row>
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.NamedFX2.uoOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-col>
                    <el-col :span="12">
                      <div class="mini-bet-button-keno height-179" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('over', gameSettings.NamedFX2.uoOdds)">
                        <el-row class="m-t-20">
      ...
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">FX 2분 배팅내역</div>
          </el-col>
        </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      baseUrl: "https://fxgame.com/fxgame/2min/index.php",   // FX 2분 중계 URL
      remainTime: "00:00",                                      // 현재 회차 남은시간
      currentGameInfo: null,                                    // 현재 회차정보
      gameName: "FX 2분",                                    // 게임명
      gameType: "NamedFX2",                                  // 게임타입
      betLogs: null,                                            // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.NamedFX2.gameStatus_auto || val.NamedFX2.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.NamedFX2.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.NamedFX2.isVisible || !this.gameSettings.Mini.named) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-NamedFX2')
    this.initframe()
    ...
    let levelSetting = this.gameSettings.NamedFX2.levelSetting[this.gameSettings.NamedFX2.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.NamedFX2.gameStatus_auto | this.gameSettings.NamedFX2.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.NamedFX2.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGameNamedFX2Round'),
    mainService.on('miniGameNamedFX2Round', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameNamedFX2Time')
    mainService.on('miniGameNamedFX2Time', (state) => {
      this.remainMins = 1
      this.remainSecs = 59
    })
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogNamedFX2')
    mainService.on('miniBetLogNamedFX2', (betLog) => {
  ...
  methods: {
    ...
    getBetNameFromType ( strTeamName ){
      var strLabel = '';
      switch (strTeamName){
        case 'buy': strLabel += '매수 (종가 맞추기)'; break;
        case 'sell': strLabel += '매도 (종가 맞추기)'; break;
