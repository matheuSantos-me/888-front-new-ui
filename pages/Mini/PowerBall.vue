  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
        ...
        <div class="panel-collapse collapse in playerbox3 minibg">
          ...
          <el-col class="mini-bet-panel">
            <el-row>
              ...
              <el-row v-if="gameSettings.PowerBall.canBetMix" style="border-top: 5px solid #f4e1c0;">
                <el-col :span="12">
              ...
              <el-row v-if="betType === 0">
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name" :class="{'height-79': !gameSettings.PowerBall.canBetNumber}">
                      {{gameSettings.PowerBall.canBetMix ? '파워볼' : '파워볼 홀짝'}}
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('power-odd', gameSettings.PowerBall.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/odd.png'"></el-row>
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('power-even', gameSettings.PowerBall.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/even.png'"></el-row>
                    </div>
                  </el-col>
                  <el-col v-if="gameSettings.PowerBall.canBetNumber" :span="getSpan()">
                    <div class="mini-bet-button" :class="[`mini-bet-button-${themeType}`]" @click="onClickBetButton('power-under', gameSettings.PowerBall.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/under.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">4.5</el-row>
                    </div>
                  </el-col>
                  <el-col v-if="gameSettings.PowerBall.canBetNumber" :span="getSpan()">
                    <div class="mini-bet-button" :class="[`mini-bet-button-${themeType}`]" @click="onClickBetButton('power-over', gameSettings.PowerBall.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/over.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">4.5</el-row>
                    </div>
                  </el-col>
                </el-row>
                <el-row v-if="!gameSettings.PowerBall.canBetNumber" class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name" :class="{'height-79': !gameSettings.PowerBall.canBetNumber}">
                      {{gameSettings.PowerBall.canBetNumber ? '파워볼' : '파워볼 언더오버'}}
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('power-under', gameSettings.PowerBall.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/under.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">4.5</el-row>
                    </div>
                  ...
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('power-over', gameSettings.PowerBall.powerBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/over.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">4.5</el-row>
                    </div>
                ...
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name" :class="{'height-79': !gameSettings.PowerBall.canBetNumber}">
                      {{gameSettings.PowerBall.canBetNumber ? '일반볼' : '일반볼 홀짝'}}
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-odd', gameSettings.PowerBall.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/odd.png'"></el-row>
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-even', gameSettings.PowerBall.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/even.png'"></el-row>
                    </div>
                  </el-col>
                  <el-col v-if="gameSettings.PowerBall.canBetNumber" :span="getSpan()">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-under', gameSettings.PowerBall.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/under.png'"></el-row>
                  ...
                  <el-col v-if="gameSettings.PowerBall.canBetNumber" :span="getSpan()">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-over', gameSettings.PowerBall.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/over.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">72.5</el-row>
                    </div>
                ...
                <el-row v-if="!gameSettings.PowerBall.canBetNumber" class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name" :class="{'height-79': !gameSettings.PowerBall.canBetNumber}">
                      {{gameSettings.PowerBall.canBetNumber ? '일반볼' : '일반볼 언더오버'}}
                    </div>
                  </el-col>
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-under', gameSettings.PowerBall.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/under.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">72.5</el-row>
                    </div>
                  ...
                  <el-col :span="getSpan()">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-over', gameSettings.PowerBall.normalBallOdds)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallOdds | filterOdds()}}</el-row>
                      <el-row :class="gameSettings.PowerBall.canBetNumber? 'm-t-12': ''"><img class="mini-bet-img" :src="'/image/mini/over.png'"></el-row>
                      <el-row class="f-w-700 mini-bet-base">72.5</el-row>
                    </div>
                ...
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile ? 24 : 4">
                    <div class="mini-bet-type-name word-keep" :class="{'height-79': !gameSettings.PowerBall.canBetNumber}">
                      일반볼 구간별
                    </div>
                  </el-col>
                  <el-col :class="isMobile ? 'width-33pro' : 'width-27pro'">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-big', gameSettings.PowerBall.normalBallSize.high)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallSize.high | filterOdds()}}</el-row>
                      <el-row class="f-s-23 f-w-700 m-t-5">대</el-row>
                      <el-row class="mini-bet-base">81-130</el-row>
                      <img class="float-left mini-bet-img-size m-l-1" :class="{'m-t-m-14': !gameSettings.PowerBall.canBetNumber}" :src="'/image/mini/big.png'">
                    </div>
                  </el-col>
                  <el-col :class="isMobile ? 'width-33pro' : 'width-27pro'">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-middle', gameSettings.PowerBall.normalBallSize.normal)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallSize.normal | filterOdds()}}</el-row>
                      <el-row class="f-s-23 f-w-700 m-t-5">중</el-row>
                      <el-row class="mini-bet-base">65-80</el-row>
                      <img class="float-left mini-bet-img-size m-l-1" :class="{'m-t-m-14': !gameSettings.PowerBall.canBetNumber}" :src="'/image/mini/middle.png'">
                    </div>
                  </el-col>
                  <el-col :class="isMobile ? 'width-33pro' : 'width-27pro'">
                    <div class="mini-bet-button" :class="[{'height-79': !gameSettings.PowerBall.canBetNumber}, `mini-bet-button-${themeType}`]" @click="onClickBetButton('normal-small', gameSettings.PowerBall.normalBallSize.low)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallSize.low | filterOdds()}}</el-row>
                      <el-row class="f-s-23 f-w-700 m-t-5">소</el-row>
                      <el-row class="mini-bet-base">15-64</el-row>
                      <img class="float-left mini-bet-img-size m-l-1" :class="{'m-t-m-14': !gameSettings.PowerBall.canBetNumber}" :src="'/image/mini/small.png'">
                    </div>
                  </el-col>
                </el-row>
                <el-row v-if="gameSettings.PowerBall.canBetNumber" class="mini-bet-row">
                  ...
                  <el-col :span="isMobile ? 24 : 20">
                    <el-row>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-0', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/0.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-1', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/1.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-2', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/2.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-3', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/3.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-4', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                    ...
                    <el-row class="m-t-10">
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-5', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/5.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-6', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/6.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-7', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/7.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-8', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
                          <el-row class="m-t-8"><img class="mini-bet-num-img" :src="'/image/mini/8.png'"></el-row>
                        </div>
                      </el-col>
                      <el-col class="width-20pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-num-9', gameSettings.PowerBall.powerBallNumber)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBallNumber | filterOdds()}}</el-row>
              ...
              <el-row v-else>
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-odd-over', gameSettings.PowerBall.powerBall_OO_EU)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBall_OO_EU | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-over.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+오버</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-odd-under', gameSettings.PowerBall.powerBall_OU_EO)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBall_OU_EO | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-under.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+언더</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-even-over', gameSettings.PowerBall.powerBall_OU_EO)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBall_OU_EO | filterOdds()}}</el-row>
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('power-even-under', gameSettings.PowerBall.powerBall_OO_EU)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.powerBall_OO_EU | filterOdds()}}</el-row>
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-over', gameSettings.PowerBall.normalBall_OO_EU)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBall_OO_EU | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-over.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+오버</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-under', gameSettings.PowerBall.normalBall_OU_EO)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBall_OU_EO | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-under.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+언더</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-over', gameSettings.PowerBall.normalBall_OU_EO)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBall_OU_EO | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/even-over.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">짝+오버</el-row>
                    </div>
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-under', gameSettings.PowerBall.normalBall_OO_EU)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBall_OO_EU | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/even-under.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">짝+언더</el-row>
                    </div>
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('odd-odd', gameSettings.PowerBall.mixBall)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.mixBall | filterOdds()}}</el-row>
                      <el-row class="m-t-12"><img :src="'/image/mini/odd-odd.png'"></el-row>
                      <el-row class="f-s-18 f-w-700">홀+홀</el-row>
                    </div>
                  </el-col>
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('odd-even', gameSettings.PowerBall.mixBall)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.mixBall | filterOdds()}}</el-row>
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('even-odd', gameSettings.PowerBall.mixBall)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.mixBall | filterOdds()}}</el-row>
                  ...
                  <el-col :span="isMobile ? 6 : 5">
                    <div class="mini-bet-button" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('even-even', gameSettings.PowerBall.mixBall)">
                      <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.mixBall | filterOdds()}}</el-row>
                  ...
                  <el-col :span="isMobile ? 24 : 20">
                    <el-row>
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-small', gameSettings.PowerBall.normalBallMix.small)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallMix.small | filterOdds()}}</el-row>
                          <el-row class="m-t-5"><img :src="'/image/mini/odd-small.png'"></el-row>
                          <el-row class="f-s-14 f-w-700">홀+소</el-row>
                        </div>
                      </el-col>
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-middle', gameSettings.PowerBall.normalBallMix.middle)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallMix.middle | filterOdds()}}</el-row>
                          <el-row class="m-t-5"><img :src="'/image/mini/odd-middle.png'"></el-row>
                          <el-row class="f-s-14 f-w-700">홀+중</el-row>
                        </div>
                      </el-col>
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-odd-big', gameSettings.PowerBall.normalBallMix.big)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallMix.big | filterOdds()}}</el-row>
                    ...
                    <el-row class="m-t-10">
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-small', gameSettings.PowerBall.normalBallMix.small)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallMix.small | filterOdds()}}</el-row>
                      ...
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-middle', gameSettings.PowerBall.normalBallMix.middle)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallMix.middle | filterOdds()}}</el-row>
                      ...
                      <el-col class="width-33pro">
                        <div class="mini-bet-button height-65" :class="`mini-bet-button-${themeType}`" @click="onClickBetButton('normal-even-big', gameSettings.PowerBall.normalBallMix.big)">
                          <el-row class="mini-bet-odds-value">{{gameSettings.PowerBall.normalBallMix.big | filterOdds()}}</el-row>
      ...
      <el-row v-show="currentUser" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`" @click="onClickHistoryTab(0)">파워볼 배팅내역</div>
          </el-col>
        </el-row>
...
export default {
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.PowerBall.gameStatus_auto || val.PowerBall.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.PowerBall.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.PowerBall.isVisible || !this.gameSettings.Mini.ntry) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-PowerBall')
    this.initframe()
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0)
    if (this.themeType == 'blue14' && this.isMobile)
      this.$store.commit("SET_MINIGAMETYPE", 8)
    else
      this.$store.commit("SET_MINIGAMETYPE", 0)

    // 게임셋팅정보 배팅슬립정보 설정
    let userLevel = this.currentUser ? this.currentUser.level : 2
    let levelSetting = this.gameSettings.PowerBall.levelSetting[this.gameSettings.PowerBall.levelSetting.findIndex(setting => setting.level == userLevel)]

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.PowerBall.gameStatus_auto | this.gameSettings.PowerBall.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.PowerBall.gameStatus_manual)
    ...
    // 미니게임 새 회차정보 패치
    mainService.off('miniGamePowerBallRound'),
    mainService.on('miniGamePowerBallRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGamePowerBallTime')
    mainService.on('miniGamePowerBallTime', (state) => {
      this.remainMins = 4
      this.remainSecs = 59
    })
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogPowerBall')
    mainService.on('miniBetLogPowerBall', (betLog) => {
  ...
  methods: {
    ...
    // span 얻기
    getSpan() {
      if (this.gameSettings.PowerBall.canBetNumber) {
