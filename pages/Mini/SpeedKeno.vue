  ...
  <div class="animated fadeInDown panel panel-inverse" :class="`panel-page-${themeType}`">
    ...
    <div class="panel-body position-relative height-full" 
              ...
              <el-row>
                <el-row class="mini-bet-row">
                  <el-col :span="isMobile? 24 : 5">
                    <div class="mini-bet-type-name" :style="isMobile  ? '' : 'height: 275px;'">
                      시작
                    </div>
                  </el-col>
                  <el-col :span="isMobile? 24 : 19">
                    <el-row>
                      <div class="mini-bet-button-keno height-135" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('odd', gameSettings.NtrySpeedKeno.OEOdds)">
                        <el-row class="m-t-20">
                        ...
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.NtrySpeedKeno.OEOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-row>
                    <el-row class="m-t-5">
                      <div class="mini-bet-button-keno height-135" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('even', gameSettings.NtrySpeedKeno.OEOdds)">
                        <el-row class="m-t-20">
                ...
                <el-row class="mini-bet-row">
                  ...
                  <el-col :span="isMobile? 24 : 19">
                    <el-row>
                      <div class="mini-bet-button-keno height-135" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('under', gameSettings.NtrySpeedKeno.UOOdds)">
                        <el-row class="m-t-20">
                        ...
                        <el-row>
                          <span class="f-s-16 mini-bet-base">{{gameSettings.NtrySpeedKeno.UOOdds | filterOdds()}}</span>
                        </el-row>
                      </div>
                    </el-row>
                    <el-row class="m-t-5">
                      <div class="mini-bet-button-keno height-135" :class="`mini-bet-button-keno-${themeType}`" @click="onClickBetButton('over', gameSettings.NtrySpeedKeno.UOOdds)">
                        <el-row class="m-t-20">
      ...
      <el-row v-if="betLogs" class="m-t-13 display-inlineblock width-full">
        <el-row>
          <el-col :span="24">
            <div class="cursor" :class="`mini-history-tab-${themeType}`">스피드키노 배팅내역</div>
          </el-col>
        </el-row>
...
const popupRouter = "NtrySpeedKeno"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      ntrySpeedKenoBet: "getNtrySpeedKenoBet",
  ...
  data() {
    return {
      ...
      baseUrl: "https://ntry.com/scores/speedkeno/live.php",   // 스피드키노 중계 URL
      remainTime: "00:00",                                    // 현재 회차 남은시간
      currentGameInfo: null,                                  // 현재 회차정보
      gameName: "스피드키노",                                  // 게임명
      gameType: "NtrySpeedKeno",                              // 게임타입
      betLogs: null,                                          // 배팅내역
      remainMins: 0,
      remainSecs: 0,
      calculateTime: null,
  ...
  watch: {
    ntrySpeedKenoBet(val) {
      this.isBetAvailable = !Boolean(val)
    },
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.NtrySpeedKeno.gameStatus_auto || val.NtrySpeedKeno.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.NtrySpeedKeno.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.NtrySpeedKeno.isVisible || !this.gameSettings.Mini.ntry) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-SpeedKeno')
    this.initframe()
    ...
    let levelSetting = this.gameSettings.NtrySpeedKeno.levelSetting[this.gameSettings.NtrySpeedKeno.levelSetting.findIndex(setting => setting.level == userLevel)]
    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.NtrySpeedKeno.gameStatus_auto | this.gameSettings.NtrySpeedKeno.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.NtrySpeedKeno.gameStatus_manual)
    ...
    mainService.off('miniGameNtrySpeedKenoRound'),
    mainService.on('miniGameNtrySpeedKenoRound', ( currentGameInfo ) => {
      if(currentGameInfo) {
    ...
    // 회차시작이벤트
    mainService.off('miniGameNtrySpeedKenoTime')
    mainService.on('miniGameNtrySpeedKenoTime', (state) => {
      this.remainMins = 4
      this.remainSecs = 59
    })
    ...
    // 배팅내역 정산후 결과 패치
    mainService.off('miniBetLogNtrySpeedKeno')
    mainService.on('miniBetLogNtrySpeedKeno', (betLog) => {
