...
export default {
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.TokenHilo.gameStatus_auto || val.TokenHilo.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.TokenHilo.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.TokenHilo.isVisible || !this.gameSettings.Mini.token) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-TokenHilo')
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0)
    this.$store.commit("SET_MINIGAMETYPE", 12)

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.TokenHilo.gameStatus_auto | this.gameSettings.TokenHilo.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.TokenHilo.gameStatus_manual)
    this.deleteBettingSlip()
  },
  methods: {
    ...
    async getInitInfo() {
      try {
        let res = await minigameInfoService.find({ query: { gameType: popupRouter, type: "TokenHilo" }})
