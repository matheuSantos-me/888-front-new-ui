...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      tokenHilo5Bet: "getTokenHilo5Bet"
    })
  },
  data() {
    return {
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.TokenHilo5.gameStatus_auto || val.TokenHilo5.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.TokenHilo5.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.TokenHilo5.isVisible || !this.gameSettings.Mini.token) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-TokenHilo5')
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0)
    this.$store.commit("SET_MINIGAMETYPE", 12)

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.TokenHilo5.gameStatus_auto | this.gameSettings.TokenHilo5.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.TokenHilo5.gameStatus_manual)
    this.deleteBettingSlip()
  },
  methods: {
    ...
    async getInitInfo() {
      try {
        let res = await minigameInfoService.find({ query: { gameType: popupRouter, type: "TokenHilo5" }})
