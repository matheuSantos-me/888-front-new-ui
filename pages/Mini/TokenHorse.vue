...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      tokenHorseBet: "getTokenHorseBet"
    })
  },
  data() {
    return {
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.TokenHorse.gameStatus_auto || val.TokenHorse.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.TokenHorse.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.TokenHorse.isVisible || !this.gameSettings.Mini.token) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-TokenHorse')
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0)
    this.$store.commit("SET_MINIGAMETYPE", 12)

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.TokenHorse.gameStatus_auto | this.gameSettings.TokenHorse.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.TokenHorse.gameStatus_manual)
    this.deleteBettingSlip()
  },
  methods: {
    ...
    async getInitInfo() {
      try {
        let res = await minigameInfoService.find({ query: { gameType: popupRouter, type: "TokenHorse" }})
