    ...
    <div class="panel-body position-relative height-full" :class="`mini-page-bg-${themeType}`" data-height="100%">
      ...
      <div class="bg-black-darker">
        <div class="panel-collapse collapse in minibg text-center">
          <iframe scrolling="no" frameborder="0" style="width: 100%; height: 900px; max-width: 1000px; margin:0px 0px 0px 0px; z-index: 1; border: none;" allowFullScreen="true" 
            :src="baseUrl"></iframe>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      tokenRacing4Bet: "getTokenRacing4Bet"
    })
  },
  data() {
    return {
  ...
  watch: {
    ...
    gameSettings(val) {
      this.isBetAvailable = !Boolean(val.gameStatus_socket || val.TokenRacing4.gameStatus_auto || val.TokenRacing4.gameStatus_manual)
      this.isCheck = !Boolean(val.gameStatus_socket || val.TokenRacing4.gameStatus_manual)
    },
    isBetAvailable(val){
  ...
  created() {
    if (!this.gameSettings.TokenRacing4.isVisible || !this.gameSettings.Mini.token) this.$router.push("/")
    
    this.getInitInfo()
  },
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'Mini-TokenRacing4')
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0)
    this.$store.commit("SET_MINIGAMETYPE", 12)

    this.isBetAvailable = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.TokenRacing4.gameStatus_auto | this.gameSettings.TokenRacing4.gameStatus_manual)
    this.isCheck = !Boolean(this.gameSettings.gameStatus_socket | this.gameSettings.TokenRacing4.gameStatus_manual)
    this.deleteBettingSlip()
  },
  methods: {
    ...
    async getInitInfo() {
      try {
        let res = await minigameInfoService.find({ query: { gameType: popupRouter, type: "TokenRacing4" }})
