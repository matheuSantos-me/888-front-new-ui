      ...
      <el-row :class="`virtual-pagein-bg-${themeType}`">
        ...
        <el-row :class="[isMobile && layoutType != 'layout2' ? `border-1-${themeType}` : '', {'p-6': layoutType != 'layout2' && !isMobile}]">
          <el-col v-if="layoutType != 'layout2'" class="virtual-league-panel">
            <div class="m-b-6" :class="[{'on': selectedLeagueName === 'brandon'}, `virtual-league-tab-${themeType}`]" style="height: 184px; padding-top: 70px;" @click="onSelectLeague('brandon')">
              브랜든 필드<br />Brandon Field
            </div>
            <div class="m-b-6" :class="[{'on': selectedLeagueName === 'stadium'}, `virtual-league-tab-${themeType}`]" style="height: 184px; padding-top: 70px;" @click="onSelectLeague('stadium')">
              노스 스타디움<br />North Stadium
            </div>
          </el-col>
          <el-row v-if="layoutType == 'layout2' && isMobile" class="p-8">
            <div class="virtual-video-tab" :class="{'on': selectedLeagueName === 'brandon'}" @click="onSelectLeague('brandon')">브랜든 필드</div>
            <div class="virtual-video-tab" :class="{'on': selectedLeagueName === 'stadium'}" @click="onSelectLeague('stadium')">노스 스타디움</div>
          </el-row>
          ...
          <el-col v-if="layoutType == 'layout2' && !isShowMobile" class="virtual-video-right p-l-8">
            <el-row>
              <el-scrollbar id="sportsLeftScrollBarId" ref="sportsLeftScrollBar" style="height: 412px;">
                <VirtualHistory :isShowMobile="isShowMobile" :gameHistory="gameHistory" :betLogs="betLogs" :historyType="historyType" :gameType="4"/>
              </el-scrollbar>
          ...
          <el-row v-if="isShowMobile && layoutType != 'layout2'" class="display-inlineblock" :class="`text-contrast-${themeType}`">
            <div class="float-left p-5 p-l-10 p-r-10 p-b-0 m-b-5 cursor">
              <div :class="selectedLeagueName === 'brandon' ? getTextColor(themeType, 1) + ' border-bottom-1-' +  themeType: ''" @click="onSelectLeague('brandon')">브랜든 필드</div>
            </div>
            <div class="float-left p-5 p-l-10 p-r-10 p-b-0 m-b-5 cursor">
              <div :class="selectedLeagueName === 'stadium' ? getTextColor(themeType, 1) + ' border-bottom-1-' +  themeType: ''" @click="onSelectLeague('stadium')">노스 스타디움</div>
            </div>
          </el-row>
        </el-row>
        ...
        >
          <VirtualBetPanel :currentGame="currentGame" :isShowMobile="isShowMobile" :isBetAvailable="isBetAvailable" :gameType="4" />
        </div>
        <!-- 왼쪽 패널 끝 -->
        <!-- 오른쪽 패널 -->
...
export default {
  ...
  data() {
    return {
      ...
      selectedLeagueName: "brandon",
      videoUrl: "https://b6.nust365.com/?vn=15&sw=648&sh=365",
      isShowMobile: false,
      gameHistory: null,
      currentGame: null,
      betLogs: null
  ...
  watch: {
    gameSettings(val) {
      this.isBetAvailable = !Boolean(
        val.gameStatus_socket || val.VBaseball.gameStatus_manual
      );
    },
    isBetAvailable(val) {
  ...
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'VirtualSports-Baseball')
    this.initframe();
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0);
    // 게임셋팅정보 배팅슬립정보 설정
    this.isBetAvailable = !Boolean(
      this.gameSettings.gameStatus_socket |
        this.gameSettings.VBaseball.gameStatus_manual
    );
    this.deleteBettingSlip();

    let userLevel = this.currentUser ? this.currentUser.level : 2;
    let levelSetting = this.gameSettings.VBaseball.levelSetting[this.gameSettings.VBaseball.levelSetting.findIndex((setting) => setting.level == userLevel)];
    ...
    // 가상축구 경기추가 패치
    mainService.off("virtualGameBaseball"),
    mainService.on("virtualGameBaseball", (patchData) => {
      this.addGame(patchData);
    });

    // 가상축구 경기상태 패치
    mainService.off("virtualGameBaseballStatus"),
    mainService.on("virtualGameBaseballStatus", (patchData) => {
      this.updateGameStatus(patchData);
    });

    // 가상축구 경기내역 패치
    mainService.off("virtualGameBaseballHistory"),
    mainService.on("virtualGameBaseballHistory", (patchData) => {
      this.updateGameHistory(patchData);
    });

    // 가상축구 경기내역 패치
    mainService.off("virtualBetLogBaseball"),
    mainService.on("virtualBetLogBaseball", (patchData) => {
  ...
  methods: {
    ...
    async getInitInfo() {
      try {
        let res = await virtualGameInfoService.find({ query: {gameType: "VBaseball" }})
        if (res.code != 1) {
    ...
    initframe() {
      ...
      if (this.selectedLeagueName === "brandon") {
        this.videoUrl = "https://b6.nust365.com/?vn=15&sw=" + width + "&sh=" + height;
      } else {
        this.videoUrl = "https://b6.nust365.com/?vn=16&sw=" + width + "&sh=" + height;
      }
    },
    // 남은시간 설정
    setReaminTime(game) {
