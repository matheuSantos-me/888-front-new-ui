      ...
      <el-row :class="`virtual-pagein-bg-${themeType}`">
        ...
        <el-row :class="[isMobile && layoutType != 'layout2' ? `border-1-${themeType}` : '', {'p-6': layoutType != 'layout2' && !isMobile}]">
          <el-col v-if="layoutType != 'layout2'" class="virtual-league-panel">
            <div
              class="m-b-6"
              :class="[{'on': selectedLeagueName === 'Baker Gardens'}, `virtual-league-tab-${themeType}`]"
              @click="onSelectLeague('Baker Gardens')"
              style="height: 184px; padding-top: 70px;"
            >
              베이커 가든스
              <br />Baker Gardens
            </div>
            <div
              class="m-b-6"
              :class="[{'on': selectedLeagueName === 'Waterport Arena'}, `virtual-league-tab-${themeType}`]"
              @click="onSelectLeague('Waterport Arena')"
              style="height: 184px; padding-top: 70px;"
            >
              워터포트 아레나
              <br />Waterport Arena
          ...
          <el-row v-if="layoutType == 'layout2' && isMobile" class="p-8">
            <div class="virtual-video-tab" :class="{'on': selectedLeagueName === 'Baker Gardens'}" @click="onSelectLeague('Baker Gardens')">베이커 가든스</div>
            <div class="virtual-video-tab" :class="{'on': selectedLeagueName === 'Waterport Arena'}" @click="onSelectLeague('Waterport Arena')">워터포트 아레나</div>
          </el-row>
          <el-col class="text-center p-t-3" :class="layoutType == 'layout2' ? `virtual-video-panel-${themeType}` : 'virtual-video-panel'">
            <el-row>
              <el-row v-if="layoutType == 'layout2' && !isMobile" style="background: #16171A; border-radius: 6px 6px 0px 0px; padding: 0 50px;">
                <div class="virtual-video-tab" :class="{'on': selectedLeagueName === 'Baker Gardens', 'width-50pro': !isMobile}" @click="onSelectLeague('Baker Gardens')">베이커 가든스</div>
                <div class="virtual-video-tab" :class="{'on': selectedLeagueName === 'Waterport Arena', 'width-50pro': !isMobile}" @click="onSelectLeague('Waterport Arena')">워터포트 아레나</div>
              </el-row>
              <iframe
                id="video-frame"
          ...
          <el-col v-if="layoutType == 'layout2' && !isShowMobile" class="virtual-video-right p-l-8">
            <el-row>
              <el-scrollbar id="sportsLeftScrollBarId" ref="sportsLeftScrollBar" style="height: 412px;">
                <VirtualHistory :isShowMobile="isShowMobile" :gameHistory="gameHistory" :betLogs="betLogs" :historyType="historyType" :gameType="1" />
              </el-scrollbar>
          ...
          <el-row v-if="isShowMobile && layoutType != 'layout2'" class="display-inlineblock" :class="`text-contrast-${themeType}`">
            <div class="float-left p-5 p-l-10 p-r-10 p-b-0 m-b-5 cursor">
              <div
                :class="selectedLeagueName === 'Baker Gardens' ? getTextColor(themeType, 1) + ' border-bottom-1-' +  themeType: ''"
                @click="onSelectLeague('Baker Gardens')"
              >베이커 가든스</div>
            </div>
            <div class="float-left p-5 p-l-10 p-r-10 p-b-0 m-b-5 cursor">
              <div
                :class="selectedLeagueName === 'Waterport Arena' ? getTextColor(themeType, 1) + ' border-bottom-1-' +  themeType: ''"
                @click="onSelectLeague('Waterport Arena')"
              >워터포트 아레나</div>
            </div>
          </el-row>
        ...
        >
          <VirtualBetPanel :currentGame="currentGame" :isShowMobile="isShowMobile" :isBetAvailable="isBetAvailable" :gameType="1" />
        </div>
        <!-- 왼쪽 패널 끝 -->
        <!-- 오른쪽 패널 -->
...
export default {
  ...
  data() {
    return {
      ...
      selectedLeagueName: "Baker Gardens",
      videoUrl: "http://b4.nust365.com/n/?vn=11&sw=648&sh=365",
      isShowMobile: false,
      gameHistory: null,
      currentGame: null,
      betLogs: null
  ...
  watch: {
    gameSettings(val) {
      this.isBetAvailable = !Boolean(
        val.gameStatus_socket || val.VBasket.gameStatus_manual
      );
    },
    isBetAvailable(val) {
  ...
  mounted() {
		this.$store.commit('SET_CURRENTPAGENAME', 'VirtualSports-Basketball')
    this.initframe();
    this.$store.commit("SET_LEFTSIDEMENUTYPE", 0);
    // 게임셋팅정보 배팅슬립정보 설정
    this.isBetAvailable = !Boolean( this.gameSettings.gameStatus_socket | this.gameSettings.VBasket.gameStatus_manual );
    this.deleteBettingSlip();

    let userLevel = this.currentUser ? this.currentUser.level : 2;
    let levelSetting = this.gameSettings.VBasket.levelSetting[ this.gameSettings.VBasket.levelSetting.findIndex( (setting) => setting.level == userLevel ) ];
    ...
    // 가상축구 경기추가 패치
    mainService.off("virtualGameBasket"),
    mainService.on("virtualGameBasket", (patchData) => {
      this.addGame(patchData);
    });

    // 가상축구 경기상태 패치
    mainService.off("virtualGameBasketStatus"),
    mainService.on("virtualGameBasketStatus", (patchData) => {
      this.updateGameStatus(patchData);
    });

    // 가상축구 경기내역 패치
    mainService.off("virtualGameBasketHistory"),
    mainService.on("virtualGameBasketHistory", (patchData) => {
      this.updateGameHistory(patchData);
    });

    // 가상축구 경기내역 패치
    mainService.off("virtualBetLogBasket"),
    mainService.on("virtualBetLogBasket", (patchData) => {
    ...
    mainService.on("virtualBetLogCreated", (betLog) => {
      if (betLog.gameType === 4 && betLog.userId === this.currentUser._id) {
  ...
  methods: {
    ...
    initframe() {
      ...
      if (this.selectedLeagueName === "Baker Gardens") {
        this.videoUrl = "http://b4.nust365.com/n/?vn=11&sw=" + width + "&sh=" + height;
      } else {
        this.videoUrl = "http://b4.nust365.com/n/?vn=12&sw=" + width + "&sh=" + height;
      }
    },
    // 남은시간계산
    setReaminTime(game) {
    ...
    // 경기추가
    addGame(patchData) {
      if (patchData.gameType !== 4) return;
    ...
    updateGameStatus(patchData) {
      if (patchData.gameType !== 4 || this.currentGame == null) return;
      let index = this.currentGame.findIndex((game) => game.leagueName === patchData.leagueName && game.eventId === patchData.eventId);
      if (index > -1) {
    ...
    updateGameHistory(patchData) {
      if (patchData.gameType !== 4 || this.gameHistory == null) return;
